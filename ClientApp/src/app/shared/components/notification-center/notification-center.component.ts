...
export class NotificationCenterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.notificationService.getUnreadCountObservable().subscribe((count: number) => {
  ...
  clearCategoryNotifications(category: string): void {
    // Clear notifications by type (since category doesn't exist in model)
    this.notifications
  ...
  get filteredNotifications(): Notification[] {
    ...
    // Filter by type (using type instead of category)
    if (this.selectedCategory !== 'all') {
      filtered = filtered.filter(n => n.type === this.selectedCategory);
    }
  ...
  get categories(): string[] {
    const cats = new Set(this.notifications.map(n => n.type).filter(Boolean));
